  ...
  <Dialog :title="dialogTitle" v-model="dialogVisible">
    ...
    >
      <el-form-item label="规格名" prop="name">
        <el-input v-model="formData.name" placeholder="请输入规格名" />
      </el-form-item>
      ...
      <el-form-item label="序列" prop="sequence">
        <el-input v-model="formData.sequence" placeholder="请输入序列" />
      </el-form-item>
     </el-form>
    <template #footer>
...
const formRules = reactive({
  ...
  categoryId: [{ required: true, message: '所属类目不能为空', trigger: 'change' }]
})
...
const open = async (type: string, id?: number, categoryId: number) => {
  ...
  if (id) {
    ...
    try {
      formData.value = await CategoryApi.getSpec(id)
    } finally {
...
const submitForm = async () => {
  ...
  try {
    ...
    if (formType.value === 'create') {
      await CategoryApi.createSpec(data)
      message.success(t('common.createSuccess'))
    } else {
      await CategoryApi.updateSpec(data)
